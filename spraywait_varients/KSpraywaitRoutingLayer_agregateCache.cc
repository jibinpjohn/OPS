...
void KSpraywaitRoutingLayer::initialize(int stage)
{
    ...
    if (stage == 0) {
        ...
        syncedNeighbourListIHasChanged = TRUE;
	         ...
	         L=par("noDuplicate");
	EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << "TheNOOFCOPIES:"<<L<<" \n";
    ...
    while (expiredFound) {
        ...
        if (expiredFound) {
          ...
          //jibin  EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << KSPRAYWAITROUTINGLAYER_DEBUG << " :: Removing Expired Data Entry :: "
            ...
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_age: "<<"cache size removed OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
            currentCacheSize -=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_age: "<<"cache size removed NEWIMP:" <<((cacheEntry->realPacketSize)*(cacheEntry->copies))<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
...
void KSpraywaitRoutingLayer::handleDataMsgFromUpperLayer(cMessage *msg)
{
    ...
    if (!found) {
        ...
        if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataupper_check: "<<"cache size added OLDIMP:"<< omnetDataMsg->getRealPacketSize()<<"Duplicates:"<<L<<"cache size added NEWIMP:"<<(omnetDataMsg->getRealPacketSize())*L<< "\n";}
        if (maximumCacheSize != 0
                && (currentCacheSize + (omnetDataMsg->getRealPacketSize()*L)) > maximumCacheSize
                && cacheList.size() > 0) {
            iteratorCache = cacheList.begin();
            ...
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataupper_remove: "<<"cache size removed OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
            currentCacheSize -=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataupper_remove: "<<"Duplicates: "<<(cacheEntry->copies)<<" cache size removed NEWIMP:" <<(cacheEntry->realPacketSize)*(cacheEntry->copies)<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
        ...
        currentCacheSize += cacheEntry->realPacketSize;
          if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataupper_add: "<<"cache size added OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
          currentCacheSize +=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);
          if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataupper_add: "<<"cache size added NEWIMP:" <<((cacheEntry->realPacketSize)*(cacheEntry->copies))<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
...
if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_datalwr_check: "<<"cache size added OLDIMP:"<< omnetDataMsg->getRealPacketSize()<<"Duplicates:"<<omnetDataMsg->getDuplicates()<<"cache size added NEWIMP:"<<omnetDataMsg->getRealPacketSize()*(omnetDataMsg->getDuplicates())<< "\n";}


            if (maximumCacheSize != 0
                && (currentCacheSize + (omnetDataMsg->getRealPacketSize()*omnetDataMsg->getDuplicates())) > maximumCacheSize
                && cacheList.size() > 0) {
      ...
      /*  if (found) {
        ...
        if (logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" << ownMACAddress << ">!<UO>!<DM>!<" << omnetDataMsg->getSourceAddress() << ">!<"
            ...
            << omnetDataMsg->getByteLength() << "\n";}
                ...
                currentCacheSize -= removingCacheEntry->realPacketSize;
                if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_datalwr_remove: "<<"cache size removed OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
                currentCacheSize -=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);
                if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_datalwr_remove: "<<"Duplicates: "<<(cacheEntry->copies)<<" cache size removed NEWIMP:" <<((cacheEntry->realPacketSize)*(cacheEntry->copies))<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
            ...
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_datalwr_add: "<<"cache size added OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
            currentCacheSize +=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);
            if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_datalwr_add: "<<"cache size added NEWIMP:" <<((cacheEntry->realPacketSize)*(cacheEntry->copies))<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
...
//if destination is found  only one copy of message is being transmitted in order to reduce metric number of transmission
	...
	//compare the source adress and destination address in cache if matches send to destination
          ...
          if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataRlwr_rmv_destfnd: "<<"cache size removed OLDIMP:" <<cacheEntry->realPacketSize<<"Total cache size OLDIMP:" <<currentCacheSize<<"\n";}
          currentCacheSize -=(cacheEntry->realPacketSize)*(cacheEntry->copies-1);

          if (cache_logging) {EV_INFO << KSPRAYWAITROUTINGLAYER_SIMMODULEINFO << ">!<" <<"CACHE_dataRlwr_rmv_destfnd: "<<"cache size added NEWIMP:" <<((cacheEntry->realPacketSize)*(cacheEntry->copies))<<"Total cache size NEWIMP:" <<currentCacheSize<<"\n";}
